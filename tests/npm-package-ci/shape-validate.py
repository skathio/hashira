#!/usr/bin/env python3
"""
Shape validation for `.github/workflows/npm-package-ci.yml`.

Acceptance check per phase 2.2 (+ iter-2.3 D14 update):
  ...
  (d) The workflow's `on.workflow_call.outputs` block declares EXACTLY the
      outputs in DOCUMENTED_OUTPUTS (D8's CI->CD one-scalar boundary
      contract) — no extra output (e.g. a `v`-prefixed tag form or the bump
      kind) is permitted, and each declared output has a non-empty
      `description:`. Additionally (pass-1 review, Major #3) each output's
      `value:` expression is validated for CONNECTIVITY, not just shape: the
      referenced job id must exist under `jobs:`, that job's own `outputs:`
      block must declare the same output name mapped from a
      `steps.<id>.outputs.<name>` expression, and a step with that exact
      `id` must exist in that job's `steps:` list. A typo'd job id, job
      output name, or step id fails this check closed instead of silently
      producing an empty output at runtime.

Run from repo root: python3 tests/npm-package-ci/shape-validate.py
Exits 0 on success; non-zero with a clear error otherwise.

Uses python3 + PyYAML which are preinstalled on ubuntu-latest. Matches
iter 2.1's local YAML-load precedent.
"""
...
WORKFLOW_PATH = os.path.join(REPO_ROOT, ".github", "workflows", "npm-package-ci.yml")

# Inline manifest for the shape check. Iter 2.3 adds `library_ref` per D14.
# Iter 2.2 adds `bump`/`seed_version` (version-resolver wiring, D8).
DOCUMENTED_INPUTS = [
    "node_version",
    "test_command",
    "coverage_path",
...
# Iter 2.2: the CI->CD output-boundary contract (D8 / phase 2 exit criteria
...
# per phase 2 exit criteria (D8's CI->CD boundary contract). Used by check
# (d)'s connectivity validation (pass-1 review, Major #3 — a typo'd job id
# previously passed the shape check while silently producing an empty output
# at runtime).
...
def collect_uses(node, acc):
    """Recursively collect every `uses:` string value in a parsed YAML tree."""
...
def validate_uses_ref(ref):
    """Return (ok, reason). Local `./` refs must point to existing paths;
    `./.hashira/...` refs (D14) must point to an action directory under
    ...
    if ref.startswith("./"):
        # Legacy local path (library self-CI only).
    ...
    # Third-party ref. Must be `owner/repo[/path]@<40-char-sha>`.
    if "@" not in ref:
...
def validate_output_connectivity(name, spec, jobs_block):
    """Check (d) connectivity extension (pass-1 review, Major #3).
    ...
    mode that slipped through (a misspelled job id or step id silently
    producing an empty `resolved_version` at runtime).

    Known limitation (pass-2 re-review, Minor NEW-1): step (4) confirms a
    step with the referenced id exists, but does NOT confirm that step's own
    output carries the expected name — that mapping lives inside the
    invoked composite action's own `action.yml` (e.g. `version-resolver`),
    outside this workflow YAML, so it isn't fully resolvable by a structural
    check against this file alone. A correct-but-wrong step id (one that
    exists but doesn't produce `<name>`), or a correct step id with a
    mistyped final output name, will NOT be caught here. The 3 typo shapes
    `--self-test` exercises (job id, job-output name, step id) are caught;
    this 4th shape is a documented gap, not silently assumed covered.
    """
...
def main():
    ...
    # Check (d): the CI->CD output-boundary contract (D8 / phase 2 exit
    # criteria "Major 3 fix") — exactly one workflow-level output, no second
    # output for the `v`-prefixed tag form or the bump kind. This check reads
    # the YAML's `outputs:` block directly, not just resolver/runtime
    # behavior, per the iteration's acceptance criteria.
    ...
    # Check (d) connectivity extension (pass-1 review, Major #3): the key
    # name + description checks above pass even if the `value:` expression
    # references a job/step id that doesn't exist (a typo silently produces
    # an empty output at runtime instead of failing the shape check). Verify
    # the full jobs.<id>.outputs.<name> <- steps.<id>.outputs.<name> chain.
    ...
    # Check workflow-level permissions deny-all baseline (NF6).
...
def run_self_test():
    """`--self-test`: typo-injection regression test for check (d)'s
    connectivity extension (pass-1 review, Major #3).

    Loads the REAL workflow file, then for each case below mutates a deep
    copy's `jobs:` block by exactly one id/name and asserts
    `validate_output_connectivity` now reports failure. Before this
    extension, all three of these typos passed check (d) cleanly while the
    real workflow would have silently produced an empty `resolved_version`
