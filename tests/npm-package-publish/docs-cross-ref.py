...
`.github/workflows/npm-package-publish.yml` (and `npm-package-ci.yml`)
appears in the corresponding markdown input table in `docs/flows/npm.md`,
and vice versa. Secrets are checked similarly.

Acceptance criterion in phase 02 iteration 2.3.

Run from repo root: python3 tests/npm-package-publish/docs-cross-ref.py
"""
...
CI_WF = os.path.join(REPO_ROOT, ".github", "workflows", "npm-package-ci.yml")
PUB_WF = os.path.join(REPO_ROOT, ".github", "workflows", "npm-package-publish.yml")
NPM_DOC = os.path.join(REPO_ROOT, "docs", "flows", "npm.md")
...
# Match table rows of the shape "| `name` | ..." where `name` is in
# backticks at the start of the first column.
ROW_NAME_RE = re.compile(r"^\|\s*`([A-Za-z_][A-Za-z0-9_]*)`\s*\|")


def parse_table_names_under_heading(md_text, heading_substring):
    """Find the first ### heading containing `heading_substring`, then
    scan forward until the next heading (## or ###) and collect names
    from `| \`name\` | ...` rows."""
    ...
    for i, ln in enumerate(lines):
        if (ln.startswith("### ") or ln.startswith("## ")) and heading_substring in ln:
            ...
            break
    if start is None:
        fail(f"could not find heading containing '{heading_substring}' in {NPM_DOC}")
    names = []
...
def main():
    ...
    with open(NPM_DOC, "r", encoding="utf-8") as f:
        md = f.read()
