#!/usr/bin/env python3
"""
Shape validation for `.github/workflows/npm-package-publish.yml`.

Acceptance check per phase 2.3:
  ...
  (e) The release job declares `environment: ${{ inputs.environment_name }}`
      (the gate per D13).

Run from repo root: python3 tests/npm-package-publish/shape-validate.py
"""
...
WORKFLOW_PATH = os.path.join(REPO_ROOT, ".github", "workflows", "npm-package-publish.yml")
...
def main():
    ...
    # Release job environment gate (D13).
    jobs = doc.get("jobs") or {}
    release = jobs.get("release")
    if not isinstance(release, dict):
        fail("missing `release` job")
    env_decl = release.get("environment")
    if not env_decl:
        fail("release job missing `environment:` declaration (D13 gate)")
    if "${{ inputs.environment_name }}" not in str(env_decl):
        fail(
            f"release job `environment:` must reference inputs.environment_name "
    ...
    print(
        ...
        "all present with non-empty descriptions)"
    )
    return 0
